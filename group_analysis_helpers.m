...
%   iterates over the Brainstorm stat files listed in STATFILES and produces
%   distribution plots plus textual/structured summaries for each. CONTEXTLABEL
%   (optional) lets the caller identify the group/contrast (e.g.,
%   "Active_Stim_vs_Pre_Night1") and is injected into output folder names and
%   metadata when supplied.
%
%   OUTPUTS is a struct array with fields:
%       - statFile            : Absolute path to the statistic file
%       - distributionFigure  : Path to the permutation distribution PNG
...
%   summarize_cluster_anatomy).
    ...
    for i = 1:numel(statFiles)
        ...
        if ~isempty(labelSlug)
            statBase = sprintf('%s_%s', labelSlug, statBase);
        end
